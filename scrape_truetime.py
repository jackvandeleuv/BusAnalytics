...
class Scraper:
    ...
    def __scrape_html_tag(self, url, tag):
        page = requests.get(url)
    ...
    def __zip_stop_id_and_name(self, url, direction, r_num, r_name):
        page = requests.get(url)
        # Specify lxml parser to avoid different default parsers on different machines
        soup_object = bs4.BeautifulSoup(page.text, features='lxml')
    ...
    def __scrape_dynamic_tag(self, url, class_name):
